...
def typ3detect(filename):
	t=open(filename,"r")
	s=open(filename+" typ3.data","w")
	count=0
	for line in t:
		if count<10 and int(line[len(line)-2])==3:
			s.write(line[len(line)-10]+line[len(line)-8]+line[len(line)-6]+line[len(line)-4]+"\n")
			count=count+1
		if count==10:
			exit()
def main(args):
	...
	#	for j in range(0,8):
	#		for k in range (0,8):
	#				if(C.subtract(i,j)==C.subtract(k,l)):
	#					#out.add(C.substract(C.subtract(i,k),C.subtract(j,l)))
	#					#print(C.substract(C.subtract(i,k),C.subtract(j,l)))
	#					x=C.subtract(i,k)
	#					y=C.subtract(j,l)
	#					print(C.subtract(x,y))
								
    
    #print(C.subtract(0,7))
    #c=Coord("fano.txt",p,o,x,y,z)
    #t=open("table","w")
    #for i in range(0,9):
	#	for j in range(0,9):
	#		t.write(str(C.subtract(i,j))+" ")
	#	t.write("\n")
	#t=open("out","w")
    #X=p.points[0][1]
    #Y=p.points[0][2]
    ...
    resultsfile = open("tempoutput" + '.res', 'w')
    prep_subtraction(p,C,resultsfile)
